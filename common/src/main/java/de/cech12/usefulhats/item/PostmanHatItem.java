...
public class PostmanHatItem extends AbstractHatItem implements IEquipmentChangeListener {
    ...
    public void appendHoverText(@NotNull ItemStack stack, @NotNull Item.TooltipContext context, @NotNull TooltipDisplay display, @NotNull Consumer<Component> tooltip, @NotNull TooltipFlag flagIn) {
        ...
        tooltip.accept(Component.translatable("item.usefulhats.postman_hat.desc.speed", UsefulHatsUtils.getRomanNumber(enchantmentLevel, false)).withStyle(ChatFormatting.BLUE));
        if (Services.CONFIG.isPostmanHatHungerEnabled()) {
            tooltip.accept(Component.translatable("item.usefulhats.postman_hat.desc.hunger").withStyle(ChatFormatting.RED));
        }
    }
    ...
    public void inventoryTick(@NotNull ItemStack stack, ServerLevel level, @NotNull Entity entity, EquipmentSlot slot) {
        if (!level.isClientSide && entity instanceof LivingEntity livingEntity) {
            ...
            if (this.isEffectCausedByOtherSource(livingEntity, MobEffects.SPEED, SPEED_DURATION, speedAmplifier)) return;
            // Speed of other sources will not be overridden here.
            if (livingEntity.isSprinting()) {
                if (livingEntity.getEffect(MobEffects.SPEED) == null || livingEntity.tickCount % 19 == 0) {
                    this.addEffect(livingEntity, MobEffects.SPEED, SPEED_DURATION, speedAmplifier);
                }
                if (Services.CONFIG.isPostmanHatHungerEnabled()) {
                ...
                this.removeEffect(livingEntity, MobEffects.SPEED, SPEED_DURATION, speedAmplifier);
                if (Services.CONFIG.isPostmanHatHungerEnabled()) {
    ...
    public void onUnequippedHatItem(LivingEntity entity, ItemStack oldStack) {
        ...
        this.removeEffect(entity, MobEffects.SPEED, SPEED_DURATION, speedAmplifier);
        if (Services.CONFIG.isPostmanHatHungerEnabled()) {
